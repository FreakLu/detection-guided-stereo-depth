...
def main():
    ...
    right_json_path = "data/json/stereo_02_R.json"
    ...
    # 打印看看哪些点进入了 Step2 匹配、代价大概多大
    for i, m in enumerate(matches):
        if m is None:
            continue
        print(f"[match] L{i} -> R{m['j']}  disp={m['disp']} z={2692.666*42.11257/m['disp']} dx={m['dx']} dy={m['dy']}  cost={m['best_cost']}  gap={m['gap']}")
